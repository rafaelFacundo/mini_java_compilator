...
public class Print {
  ...
  void prStm(SEQ s, int d) {
      indent(d); sayln("SEQ("); prStm(s.s1,d+1); sayln(",");
      prStm(s.s2,d+1); say(")");
  }
  ...
  void prStm(JUMP s, int d) {
      indent(d); sayln("JUMP("); prExp(s.e, d+1); say(")");
  }
  ...
  void prStm(CJUMP s, int d) {
	...
	default:
       ...
       sayln(","); prExp(s.e1,d+1); sayln(",");
       prExp(s.e2,d+1); sayln(",");
       indent(d+1); say(s.t.toString()); say(","); 
       say(s.f.toString()); say(")");
  }
  ...
  void prExp(BINOP e, int d) {
	...
	default:
      ...
      sayln(",");
      prExp(e.e1,d+1); sayln(","); prExp(e.e2,d+1); say(")");
   }
  ...
  void prExp(NAME e, int d) {
     indent(d); say("NAME "); say(e.cnst.toString());
  }

  void prExp(CONST e, int d) {
     indent(d); say("CONST "); say(String.valueOf(e.cnst));
  }

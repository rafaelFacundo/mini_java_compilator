  ...
  void trace(Tree.StmList l) {
   for(;;) {
	...
	Tree.JUMP j = (Tree.JUMP)s;
        Tree.StmList target = (Tree.StmList)table.get(j.labels.head);
	if (j.labels.tail==null && target!=null) {
               last.tail=target;
	       l=target;
        }
	else {
	...
	Tree.CJUMP j = (Tree.CJUMP)s;
        Tree.StmList t = (Tree.StmList)table.get(j.t);
        Tree.StmList f = (Tree.StmList)table.get(j.f);
        if (f!=null) {
	  last.tail.tail=f; 
	  l=f;
	}
        else if (t!=null) {
	  last.tail.head=new Tree.CJUMP(Tree.CJUMP.notRel(j.operador),
					j.e1,j.e2,
					j.f,j.t);
	  last.tail.tail=t;
	  l=t;
	  ...
	  last.tail.head=new Tree.CJUMP(j.operador,j.e1,j.e2,
					j.t,ff);
	  last.tail.tail=new Tree.StmList(new Tree.LABEL(ff),
		           new Tree.StmList(new Tree.JUMP(j.f),
					    getNext()));
	  return;
        }
     }
